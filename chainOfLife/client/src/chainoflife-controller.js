...
class ChainOfLifeController {
    ...
    async join(_gameId, _field) {
        await this.executeMessage('join', [_gameId, _field])
    }

    async resolve(_gameId, _field, _isWinner) {
        await this.executeMessage('resolve', [_gameId, _field, _isWinner])
    }

    async finalize(_gameId) {
        await this.executeMessage('finalize', [_gameId])
    }
}
